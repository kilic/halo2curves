...
pub(crate) fn impl_arith(
    ...
    let impl_from_mont = impl_from_mont(field, num_limbs, modulus_limbs, inv);
...
fn impl_mont(field: &syn::Ident, num_limbs: usize, modulus_limbs: &[u64], inv: u64) -> TokenStream {
    ...
    (0..num_limbs).for_each(|i| {
        ...
        let modulus_limb = modulus_limbs[i];
        gen.extend(quote! { let (#d_i, borrow) = sbb(#r_in, #modulus_limb, #borrow); });
    });
...
fn impl_from_mont(
    ...
    for i in 0..num_limbs {
        ...
        for (j, m) in modulus_limbs.iter().enumerate() {
            ...
            gen.extend(quote! { let (#r_out, #r_i) = mac(#r_ij, k, #m, #carry_in); });
        }
    }
    let ret: Vec<_> = (0..num_limbs).map(|i| fmtid!("r_{}", i)).collect();
    let modulus = quote! {const MODULUS : #field = #field([#(#modulus_limbs,)*]);};

    quote! {
        #gen
        #modulus
        #field([#(#ret),*]).sub(&MODULUS).0
    }
}
