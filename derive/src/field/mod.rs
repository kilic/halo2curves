...
pub(crate) fn impl_field(input: TokenStream) -> TokenStream {
    ...
    let impl_field = quote! {
        ...
        impl #field {
            ...
            pub(crate) const MODULUS: [u64; Self::NUM_LIMBS] = #modulus_limbs_ident;
            ...
            pub(crate) fn is_less_than_modulus(limbs: &[u64; Self::NUM_LIMBS]) -> bool {
                let borrow = limbs.iter().enumerate().fold(0, |borrow, (i, limb)| {
                    crate::arithmetic::sbb(*limb, Self::MODULUS[i], borrow).1
                });
                (borrow as u8) & 1 == 1
            }
        }
    ...
    let impl_arith = {
        if num_limbs == 4 && num_bits < 256 {
            ...
            arith::impl_arith(&field, num_limbs, &modulus_limbs, inv64)
        }
    };
    #[cfg(not(feature = "asm"))]
    let impl_arith = arith::impl_arith(&field, num_limbs, &modulus_limbs, inv64);

    let impl_arith_always_const =
        arith::impl_arith_always_const(&field, num_limbs, &modulus_limbs, inv64);
